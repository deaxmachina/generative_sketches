<script>
  ...
  let geometry
  let meshes = []
  let materialBg = undefined 
  let meshBg = undefined
  let ambientLight
  ...
  onMount(() => {
    ...
    // not available yet
    const wrapper = document.getElementById('many-cubes-wrapper')
    ...
    ************************************/
    ...
    materialBg = new THREE.MeshStandardMaterial({
      color: new THREE.Color('#fff'),
      side: THREE.DoubleSide
    })

    planeGeometry = new THREE.PlaneGeometry(13, 13)
    plane = new THREE.Mesh(planeGeometry, materialBg)
    ...
    ************************************/
    ...
    // Hemisphere light
    hemisphereLight = new THREE.HemisphereLight('#3a86ff', '#7b1e7a', 0.9)
  ...
  $: if (meshes, geometry, material) {
    ...
    meshes.forEach((mesh, i) => {
      ...
      const scale = noise2D(i, i*Math.random())
      //mesh.scale.set(noise2D(i, i*Math.random()), noise2D(i, i*Math.random()), noise2D(i, i*Math.random()))
...
<div class=wrapper>
  <div id='many-cubes-wrapper' bind:clientWidth="{widthContainer}">
