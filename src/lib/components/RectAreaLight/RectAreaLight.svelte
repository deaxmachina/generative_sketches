<script>
  ...
  export let noise2D
  export let rectLightIntensity
  export let rectLightWidth
  export let rectLightHeight
  ...
  $: height = windowWidth > 1000 ? widthContainer * 1 : widthContainer * 1.2
  $: aspect = width / height
  ...
  onMount(() => {
    ...
    ************************************/
    ...
    for (let i = 0; i < numMeshes; i ++) {
      ...
      mesh.position.set(
        noise2D(i, i*2)*2, Math.max(0, noise2D(i, i*3)*2), Math.max(0, noise2D(i, i*5))
      )
      const scale = noise2D(i, i) * 0.4
    ...
    ************************************/
    ...
    const tick = () => {
      ...
      for (let i = 0; i < meshes.length; i++) {
        const mesh = meshes[i]
        mesh.position.z = (Math.sin(elapsedTime))*noise2D(i, i)*0.85
      }
...
<div class='wrapper' id='rect-area-wrapper' bind:clientWidth="{widthContainer}" bind:clientHeight="{heightContainer}">
